...
Why this model:
Logistic regression seems like the best choice because it performs well and is still easy to interpret. Its performance is close to GAM, but the coefficients are more straightforward to explain.

What the company can responsibly conclude:
The company can use the model to see which customer features are related to higher or lower churn risk.
...
We would test the model on new customer data to see if the results and patterns stay consistent.
